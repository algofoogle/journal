...
typedef union {
    struct __attribute__((packed)) {
        ...
        uint64_t gpout3_sel             : 6; // Split from first 32 bits to second, here.
...
#define update_rbz_hi(  p)             { reg_la3_data=(p).w.la3; }
#define update_rbz_lo(  p)             { reg_la2_data=(p).w.la2; }
#define update_rbz(     p)             { update_rbz_lo(p); update_rbz_hi(p); }
#define spi_init(       spi, la)       { (la).f.spi##_csb=1;   (la).f.spi##_sclk=0;                            update_rbz_lo(la); (la).f.spi##_csb=0;                      update_rbz_lo(la); }
#define spi_send_bit(   spi, la, bit)  {                       (la).f.spi##_sclk=0; (la).f.spi##_mosi=(bit);   update_rbz_lo(la);                     (la).f.spi##_sclk=1; update_rbz_lo(la); }
#define spi_stop(       spi, la)       { (la).f.spi##_csb=0;   (la).f.spi##_sclk=0;                            update_rbz_lo(la); (la).f.spi##_csb=1;                      update_rbz_lo(la); }
...
void main()
{
    ...
    reg_hkspi_pll_source  = 0b000000;   // 000=div-1 (50MHz)
    ...
    while (1) {
        ...
        if (!uart_rxempty_read()) {
            ...
            // while (reg_uart_txfull == 1); reg_uart_data = 'V';
            ...
            if (10==control_state) {
                ...
                spi_stop(vec, la);
                control_state = 0;
                // Wait until transmit buffer is clear, then send 'V':
                reg_gpio_out = reg_gpio_out ^ 1;
                while (reg_uart_txfull == 1); reg_uart_data = 'V'; // print("V");
            }
            else if (16==control_state) {
                // REG stream end.
                spi_stop(reg, la);
                control_state = 0;
                // Wait until transmit buffer is clear, then send 'R':
                while (reg_uart_txfull == 1); reg_uart_data = 'R'; // print("R");
            }
        }
    }
}
