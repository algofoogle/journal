...
2.  Clone the repo
3.  Add [raybox-zero] tag `1.0` (we might change it later) as a submodule:
    ```bash
...
*   **Complete doco on updated tests now in `test/` and not `src/test/`**
*   Document enabling GitHub Pages publishing via GHA.
*   Change uo_outs to match Tiny VGA PMOD, or optional switching (e.g. Matt's clock version).
