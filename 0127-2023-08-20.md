...
|-|-|
| [**0126**-2023-08-19.md](./0126-2023-08-19.md) | [**0128**-2023-08-21.md](./0128-2023-08-21.md) |

# raybox-zero

## In progress: Trying to fix GDS hardening
...
## A new TT04 test repo: Reciprocal

*   Let's take the `lzc` and `reciprocal` modules and put them in their own TT04 test.
*   This will help us work out how many tiles they would actually use up, and how many standard cells.
*   If they succeed in hardening using the TT04 GHAs, then we can assume negative vector bit indices are OK ([as the IEEE 1364-2001 spec says they should be](https://inst.eecs.berkeley.edu//~cs150/fa06/Labs/verilog-ieee.pdf#page=36)), and perhaps try working up in complexity from there. We could even assemble a test repo made out of many git submodules.
*   If it fails in yosys-abc again, we need to try creating a simpler negative-index example and see if that's the problem, or something else about the reciprocal/lzc design.

To make the new [tt04-reciprocal] repo, I followed my own guide in [0119](./0119-2023-07-22.md).



## Next steps

*   Flesh out the [tt04-reciprocal] test repo and try hardening it.
*   Options to conditionally exclude chunks of the design that might allow us to narrow down the yosys issue. This is difficult, because cutting out most things will take a whole bunch of other stuff out with them anyway; they're somewhat interdependent.
